...
import { PoshSimulatorClient } from '@/components/feature/posh-simulator/posh-simulator-client'
import { Lock, ArrowUpCircle } from 'lucide-react'
import { getOrgPlan, tierAtLeast } from '@/lib/plan'

export default async function PoshSimulatorPage() {
    ...
    const { data: userData } = await supabase
        .from('users')
        .select('department_id, organization_id, departments(posh_simulator_enabled)')
        .eq('id', user.id)
        .single()
            ...
            return (
                <div style={{ minHeight: '100vh', backgroundColor: '#0B0F19' }}
                    ...
                    <div className="max-w-md w-full text-center space-y-4 rounded-2xl border border-amber-500/30 bg-amber-500/5 p-10">
                        ...
                        <p className="text-slate-400">The POSH Simulator requires the <span className="text-amber-300 font-medium">Professional</span> plan or higher.</p>
                        <p className="text-slate-500 text-sm">
                            Contact{' '}
    ...
    const dept = Array.isArray(deptRaw) ? deptRaw[0] : deptRaw as { posh_simulator_enabled: boolean } | null
    const isLocked = dept !== null && dept !== undefined && dept.posh_simulator_enabled === false
    ...
    return (
        <div style={{
            ...
            backgroundImage: `
                ...
                radial-gradient(at 50% 100%, rgba(236,72,153,0.08) 0, transparent 50%)
            `,
            ...
            <div style={{
                ...
                <div className="text-center">
                    <div className="font-semibold text-white text-sm">POSH Simulator</div>
                    <div className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>
                        POSH Complaint Flow Decision Trainer
                    </div>
                </div>
                <Link
                    href="/dashboard"
                    ...
                    <div className="flex flex-col items-center gap-6 max-w-md text-center">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-slate-400 text-sm leading-relaxed">
                                The POSH Simulator is not available for your department. Please contact your administrator if you believe this is an error.
                            </p>
                        </div>
                        <Link
                            href="/dashboard"
                            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-pink-600 hover:bg-pink-500 text-white text-sm font-medium transition-colors"
                        >
                            ← Back to Dashboard
                        </Link>
                    </div>
